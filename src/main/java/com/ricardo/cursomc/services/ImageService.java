...
public class ImageService {
	...
	public BufferedImage cropImageEmQuadro(BufferedImage image) { // Pega o menor tamanho da imagem
		int minimo = (image.getWidth() <= image.getHeight()) ? image.getWidth() : image.getHeight();

		// Realiza o crop da imagem
		return Scalr.crop(image, (image.getWidth() / 2 - (minimo / 2)), (image.getHeight() / 2 - (minimo / 2)), minimo,
				minimo);
	}
